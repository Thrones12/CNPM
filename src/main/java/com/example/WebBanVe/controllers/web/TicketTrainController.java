...
public class TicketTrainController {
	...
	private String getTicketTrain(@RequestParam(defaultValue = "0") Long departure_id,
		...
		model.addAttribute("pageName", "train");
		List<Station> departures = stationService.getByType(eTransportType.TRAIN);
		if (departure_id == 0)
			departure_id = departures.get(0).getId();
		model.addAttribute("departures", departures);
		List<Station> arrivals = stationService.getByType(eTransportType.TRAIN);
		if (arrival_id == 0)
			arrival_id = arrivals.get(0).getId();
		model.addAttribute("arrivals", arrivals);

		List<Object[]> routes = routeService.search(departure_id, arrival_id, departure_time, eTransportType.TRAIN);
